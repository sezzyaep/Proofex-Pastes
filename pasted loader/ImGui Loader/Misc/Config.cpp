...
CConfig::CConfig(const char* szName, const char* regloc)
{
    this->pReg = new CLiteRegeditEasy(HKEY_CURRENT_USER, regloc);

    iActiveKey = VK_INSERT;
    bDarkBackground = true;
    bBackgroundParticles = true;

    // iActiveKey
    if (pWinFuncs->CheckRegistryValueExists(regloc, "iActiveKey", REG_DWORD))
    {
        this->iActiveKey = this->pReg->ReadInteger("iActiveKey");
    }
    // bDarkBackground
    if (pWinFuncs->CheckRegistryValueExists(regloc, "bDarkBackground", REG_DWORD))
    {
        this->bDarkBackground = this->pReg->ReadInteger("bDarkBackground");
    }
    // bBackgroundParticles
    if (pWinFuncs->CheckRegistryValueExists(regloc, "bBackgroundParticles", REG_DWORD))
    {
        this->bBackgroundParticles = this->pReg->ReadInteger("bBackgroundParticles");
    }
...
void CConfig::Update()
{
    if (!pReg->IsError())
    {
        this->pReg->WriteInteger("iActiveKey", this->iActiveKey);
        this->pReg->WriteInteger("bDarkBackground", this->bDarkBackground);
        this->pReg->WriteInteger("bBackgroundParticles", this->bBackgroundParticles);
    }
}
...
static void to_json(json& j, const stConfig::stAimbot& o, const stConfig::stAimbot& dummy = {})
{
    ...
    WRITE("iSharedRangeConfig", iSharedRangeConfig);
    WRITE("iSharedSilentConfig", iSharedSilentConfig);
    WRITE("iSharedSmoothConfig", iSharedSmoothConfig);
...
{
    WRITE("bGodMode", bGodMode);
    WRITE("bDamageProof", bDamageProof);
    WRITE("bBulletProof", bBulletProof);
    WRITE("bCollisionProof", bCollisionProof);
    WRITE("bExplosionProof", bExplosionProof);
    WRITE("bFireProof", bFireProof);
    WRITE("bMeleeProof", bMeleeProof);
...
static void to_json(json& j, const stConfig::stVehicles& o, const stConfig::stVehicles& dummy = {})
{
    WRITE("bStopOnExitVehicle", bStopOnExitVehicle);
    WRITE("iStopOnExitVehicle", iStopOnExitVehicle);
    WRITE("bAutoBikeSpam", bAutoBikeSpam);
    WRITE("iAutoBikeSpamKey", iAutoBikeSpamKey);
}

static void to_json(json& j, const stConfig::stVisuals& o, const stConfig::stVisuals& dummy = {})
{
    WRITE("bGameVisualsChanger", bGameVisualsChanger);
    WRITE("bDrawDistanceChanger", bDrawDistanceChanger);
    WRITE("bFogDistanceChanger", bFogDistanceChanger);
    WRITE("bLodDistanceChanger", bLodDistanceChanger);
    ...
    WRITE("fDrawDistance", fDrawDistance);
    WRITE("fFogDistance", fFogDistance);
    WRITE("fLodDistance", fLodDistance);
    ...
    WRITE("iTime", iTime);
    WRITE("iChamsStyle", iChamsStyle);
    WRITE("iESPHeadStyle", iESPHeadStyle);
}
...
static void from_json(const json& j, stConfig::stAimbot& r)
{
    ...
    read(j, "iSharedRangeConfig", r.iSharedRangeConfig);
    read(j, "iSharedSilentConfig", r.iSharedSilentConfig);
    read(j, "iSharedSmoothConfig", r.iSharedSmoothConfig);
...
{
    read(j, "bGodMode", r.bGodMode);
    read(j, "bDamageProof", r.bDamageProof);
    read(j, "bBulletProof", r.bBulletProof);
    read(j, "bCollisionProof", r.bCollisionProof);
    read(j, "bExplosionProof", r.bExplosionProof);
    read(j, "bFireProof", r.bFireProof);
    read(j, "bMeleeProof", r.bMeleeProof);
...
static void from_json(const json& j, stConfig::stVehicles& r)
{
    ...
    read(j, "iAutoBikeSpamKey", r.iAutoBikeSpamKey);
}

static void from_json(const json& j, stConfig::stVisuals& r)
{
    read(j, "bGameVisualsChanger", r.bGameVisualsChanger);
    read(j, "bDrawDistanceChanger", r.bDrawDistanceChanger);
    read(j, "bFogDistanceChanger", r.bFogDistanceChanger);
    read(j, "bLodDistanceChanger", r.bLodDistanceChanger);
    ...
    read(j, "fDrawDistance", r.fDrawDistance);
    read(j, "fFogDistance", r.fFogDistance);
    read(j, "fLodDistance", r.fLodDistance);
    ...
    read(j, "bPickupsESP", r.bPickupsESP);
    ...
    read(j, "iChamsStyle", r.iChamsStyle);
    read(j, "iESPHeadStyle", r.iESPHeadStyle);
}
...
void CConfig::Load(size_t index)
{
    ...
    read<value_t::object>(j, "Vehicles", g_Config.g_Vehicles);
...
void CConfig::Save(size_t index)
{
    ...
    if (std::ofstream out{ Path / (const char8_t*)Configs[index].c_str() }; out.good()) 
    {
        ...
        j["Vehicles"] = g_Config.g_Vehicles;
